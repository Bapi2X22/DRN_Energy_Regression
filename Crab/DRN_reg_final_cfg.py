...
triton_command = ( 
        ...
        "nohup tritonserver --model-repository models/ --http-port 9000 --grpc-port 9001 --metrics-port 9002 --allow-http=1 --log-verbose=1 > triton.log 2>&1 & "
        "$host = hostname"
    )   
subprocess.run(triton_command, shell=True, check=True)
    
print("Waiting for Triton to start...")
time.sleep(20)  # Give some time for the server to start
print("Triton server is running.")
"""
"""
triton_command = [
    "singularity", "exec",
    "/cvmfs/unpacked.cern.ch/registry.hub.docker.com/fastml/triton-torchgeo:22.07-py3-geometric",
    "tritonserver",
    "--model-repository=/tmp/models/",
    "--http-port=9000",
    "--grpc-port=9001",
    "--metrics-port=9002",
    "--allow-http=1",
...
# Open a log file to capture stdout/stderr
logfile = open("triton.log", "w")

# Start the server in background
triton_proc = subprocess.Popen(triton_command, stdout=logfile, stderr=logfile)

print("Waiting for Triton to start...")
time.sleep(20)  # wait for the server to fully initialize
print("Triton server is running.")
"""
...
process.TritonService.servers.append(
    cms.PSet(
        ...
        port = cms.untracked.uint32(7001),
...
process.TFileService = cms.Service("TFileService",
     ...
     fileName = cms.string("output_crab.root"),
     #fileName = cms.string('Skimmed'+'/'+ options.datasetname+'/'+infilename),
     #fileName = cms.string('/eos/user/s/sosaha/EGM_test_Sonic/CMSSW_13_3_3/src/Skimmed/Photon_model'+'/'+ options.datasetname+'/'+infilename),
     #fileName = cms.string('/eos/user/s/sosaha/EGM_test_Sonic/CMSSW_13_3_3/src/Skimmed/Electron_model'+'/'+ options.datasetname+'/'+infilename),
