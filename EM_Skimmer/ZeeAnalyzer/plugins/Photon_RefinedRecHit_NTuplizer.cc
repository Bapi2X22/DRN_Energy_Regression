...
Photon_RefinedRecHit_NTuplizer::Photon_RefinedRecHit_NTuplizer(const edm::ParameterSet& iConfig):
   ...
   drnEnergyToken_(consumes<edm::ValueMap<std::pair<float,float>>>(iConfig.getParameter<edm::InputTag>("gedPhotonsDRN"))),
...
*/
   for (size_t i = 0; i < photons->size(); ++i){
      ...
      const auto ele = photons->ptrAt(i);
/*      
...
//-----------------------------------------------Gen_Matching-----------------------------------------------------------
	...
	if (!passSuperCluster) continue;
	    ...
	    edm::Ptr<reco::Photon> elePtr = photons->ptrAt(i);
            const auto& drnValues = (*drnEnergyHandle)[elePtr];
            float recoDRNEnergy = drnValues.first;  // DRN calibrated energy
            recoDRNEnergy_.push_back(recoDRNEnergy);
...
//-----------------------------------------------------------------------------------------------------------------------
	 ...
	 GetESPlaneRecHits(*sc,theGeometry , nElectrons_, 1);
      ...
      Ele_energy_error_.push_back( ele-> getCorrectedEnergyError(reco::Photon::P4type::regression2));
      Ele_ecal_mustache_energy_.push_back( ele-> getCorrectedEnergy(reco::Photon::P4type::regression2));
      Ele_R9.push_back(ele->full5x5_r9());
      Ele_SigIEIE.push_back(ele->full5x5_sigmaIetaIeta());
//      Ele_SigIPhiIPhi.push_back(ele->full5x5_sigmaIetaIeta());
      Ele_SigIPhiIPhi.push_back(ele->full5x5_showerShapeVariables().sigmaIphiIphi);
      ...
      // Ele_sumChargedHadronPt.push_back(ele->IsolationVariables().sumChargedHadronPt);
      // Ele_sumChargedParticlePt.push_back(ele->IsolationVariables().sumChargedParticlePt);
      // Ele_sumEcalClusterEt.push_back(ele->IsolationVariables().sumEcalClusterEt);
      // Ele_sumHcalClusterEt.push_back(ele->IsolationVariables().sumHcalClusterEt);
      // Ele_sumNeutralHadronEt.push_back(ele->IsolationVariables().sumNeutralHadronEt);
      // Ele_sumPhotonEt.push_back(ele->IsolationVariables().sumPhotonEt);
      // Ele_sumPUPt.push_back(ele->IsolationVariables().sumPUPt);
      // Ele_EcalPFClusterIso.push_back(ele->ecalPFClusterIso());
      // Ele_HcalPFClusterIso.push_back(ele->hcalPFClusterIso());

      //   Pho_PFChIso.push_back(ele->sumPhotonEt());
      //   //Pho_PFChPVIso.push_back(pho->chargedHadronPFPVIso());
      //   Pho_PFPhoIso.push_back(ele->sumChargedHadronPt());
      //   Pho_PFNeuIso.push_back(ele->sumNeutralHadronEt());
      ...
      //   Pho_EcalPFClusterIso.push_back(ele->ecalPFClusterIso());
      //   Pho_HcalPFClusterIso.push_back(ele->hcalPFClusterIso());

        Pho_PFChIso.push_back(ele->photonIso());
        Pho_PFChPVIso.push_back(ele->chargedHadronPFPVIso());
        Pho_PFPhoIso.push_back(ele->chargedHadronIso());
        Pho_PFNeuIso.push_back(ele->neutralHadronIso());
        Pho_PFChWorstVetoIso.push_back(ele->chargedHadronWorstVtxGeomVetoIso());
        Pho_PFChWorstIso.push_back(ele->chargedHadronWorstVtxIso());
        Pho_EcalPFClusterIso.push_back(ele->ecalPFClusterIso());
        Pho_HcalPFClusterIso.push_back(ele->hcalPFClusterIso());
...
void Photon_RefinedRecHit_NTuplizer::GetESPlaneRecHits(const SuperCluster& sc, const CaloGeometry& theGeometry, unsigned int elenum, unsigned int planeIndex) {
